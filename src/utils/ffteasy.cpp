...
void fftc1(letype f[], int N, int skip, int forward);
void fftcn(letype f[], int ndims, int size[], int forward);
void fftr1(letype f[], int N, int forward);
void fftrn(letype f[], letype fnyquist[], int ndims, int size[], int forward);
  ...
  for(dim=ndims-1;dim>=0;dim--) /* Loop over dimensions */
  {
    ...
    for(i=0;i<totalsize;i+=planesize) /* Take big steps to begin loops of transforms */
      for(j=0;j<skip;j++) /* Skip sets the number of transforms in between big steps as well as the skip between elements */
        fftc1(f+2*(i+j),size[dim],skip,forward); /* 1-D Fourier transform. (Factor of two converts complex index to letype index.) */

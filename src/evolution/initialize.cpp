...
void set_mode(letype p2, letype m2, letype field[], letype *deriv, int real)
{
  ...
  if(p2+m2>0.) // Check to avoid letypeing point errors
...
void initialize()
{
  ...
  letype initial_field_values[nflds]; // Initial value of fields (set to zero unless specified in parameters.h)
  ...
  if(continue_run>0 && (old_grid_=fopen("grid.img","rb")))
  {
    ...
    fread_res = fread(&a, sizeof(a), 1, old_grid_);
    fread_res = fread(&ad, sizeof(ad), 1, old_grid_);
    fread_res = fread(f, sizeof(letype), nflds*gridsize, old_grid_);
    fread_res = fread(fd, sizeof(letype), nflds*gridsize, old_grid_);
    fclose(old_grid_);
  ...
  for(fld = 0; fld < nflds; fld++)
  {
    ...
    if(fld < (int)(sizeof initfield / sizeof(letype))) // Use preset values for initial field values if given
    ...
    if(fld < (int)(sizeof initderivs/sizeof(letype))) // Use preset values for initial field derivative values if given
  ...
  if(expansion > 0)
  {
    ...
    //hubble_init = 0.000365148; //TODO ONLY FOR TESTING PUPROSES
  ...
  for(fld=0;fld<nflds;fld++) // Set initial conditions for each field
  {
    ...
    set_mode(p2,mass_sq[fld], &f[fld][index(1)], &fd[fld][index(1)], 1); // The last argument specifies that a real value should be set
    ...
    for(k=1;k<N/2;k++)
    {
      ...
      set_mode(p2,mass_sq[fld], &f[fld][index(2*k)], &fd[fld][index(2*k)], 0); // Set mode
    }

    f[fld][index(0)] = 0.; // Set zeromode of field and derivative to zero (it gets added in position space)
    fd[fld][index(0)] = 0.;
    ...
    if(seed>=0)
    {
      fftr1((letype *)f[fld], N, -1); // Inverse Fourier transform of field
      fftr1((letype *)fd[fld], N, -1); // Inverse Fourier transform of field derivatives
    }
#elif NDIMS == 2
    // Loop over gridpoints.
    ...
    for(j = 0; j < N; j++)
    {
      ...
      for(k = 1; k < N/2; k++)
      {
        ...
        set_mode(p2, mass_sq[fld], &f[fld][index(2*k, j)], &fd[fld][index(2*k, j)], 0); // Set mode
      }
      ...
      if(j > N/2) // The complex conjugates of these modes appear explicitly on the lattice and must be set to satisfy f(-p)=f(p)*
      {
        ...
        set_mode(p2,mass_sq[fld], &f[fld][index(0, j)], &fd[fld][index(0, j)], 0); // Set mode
        f[fld][index(0, jconj)] =  f[fld][index(0, j)]; // Set complex conjugate mode
        f[fld][index(1, jconj)] = -f[fld][index(1, j)];
        fd[fld][index(0, jconj)] =  fd[fld][index(0, j)];
        fd[fld][index(1, jconj)] = -fd[fld][index(1, j)];
        // k=N/2
      ...
      else if(j == 0 || j == N/2) // The 4 "corners" of the lattice are set to real values
      {
        ...
        if(p2 > 0.) // Don't set the zeromode here (see below)
          set_mode(p2, mass_sq[fld], &f[fld][index(0, j)], &fd[fld][index(0, j)], 1); // The last argument specifies that a real value should be set
    ...
    f[fld][index(0, 0)] = 0.; // Set zeromode of field and derivative to zero (it gets added in position space)
    f[fld][index(1, 0)] = 0.;
    fd[fld][index(0, 0)] = 0.;
    fd[fld][index(1, 0)] = 0.;
    ...
    if(seed >= 0)
    {
      fftrn((letype *)f[fld],(letype *)fnyquist,NDIMS,arraysize,-1); // Inverse Fourier transform of field
      fftrn((letype *)fd[fld],(letype *)fdnyquist,NDIMS,arraysize,-1); // Inverse Fourier transform of field derivative
    }
#elif NDIMS == 3
    // Loop over gridpoints.
    ...
    for(i=0;i<N;i++)
    {
      ...
      for(j=0;j<N;j++)
      {
        ...
        for(k = 1; k < N/2; k++)
        {
          ...
          set_mode(p2, mass_sq[fld], &f[fld][index(2*k, j, i)], &fd[fld][index(2*k, j, i)], 0); // Set mode
       }
        ...
        if(j>N/2 || (i>N/2 && (j==0 || j==N/2))) // The complex conjugates of these modes appear explicitly on the lattice and must be set to satisfy f(-p)=f(p)*
        {
          ...
          set_mode(p2, mass_sq[fld], &f[fld][index(0, j, i)], &fd[fld][index(0, j, i)], 0); // Set mode
          f[fld][index(0, jconj, iconj)] =  f[fld][index(0, j, i)]; // Set complex conjugate mode
          f[fld][index(1, jconj, iconj)] = -f[fld][index(1, j, i)];
          fd[fld][index(0, jconj, iconj)] =  fd[fld][index(0, j, i)];
          fd[fld][index(1, jconj, iconj)] = -fd[fld][index(1, j, i)];
          // k=N/2
        ...
        else if((i == 0 || i == N/2) && (j == 0 || j == N/2)) // The 8 "corners" of the lattice are set to real values
        {
          ...
          if(p2 > 0.) // Don't set the zeromode here (see below)
            set_mode(p2, mass_sq[fld], &f[fld][index(0, j, i)], &fd[fld][index(0, j, i)], 1); // The last argument specifies that a real value should be set
    ...
    f[fld][index(0, 0, 0)] = 0.; // Set zeromode of field and derivative to zero (it gets added in position space)
    f[fld][index(1, 0, 0)] = 0.;
    fd[fld][index(0, 0, 0)] = 0.;
    fd[fld][index(1, 0, 0)] = 0.;
    ...
    {
      fftrn((letype *)f[fld], (letype *)fnyquist, NDIMS, arraysize, -1); // Inverse Fourier transform of field
      fftrn((letype *)fd[fld], (letype *)fdnyquist, NDIMS, arraysize, -1); // Inverse Fourier transform of field derivatives
    }
#endif
